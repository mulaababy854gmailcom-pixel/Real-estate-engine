...
def zoning_score(parcel_row):
    # Placeholder until zoning data is available
    return 50
...
with tab_buildability:
    st.header("🧱 Flint Buildability Engine")

    st.write("This tab scores all Flint parcels for buildability using parcel size, land use, buildings, and a zoning placeholder.")

    if st.button("Run Buildability Scoring"):
        with st.spinner("Scoring parcels..."):
            parcels_3857, parcels_4326, top50 = score_parcels()

        st.success("Buildability scoring complete.")

        st.subheader("Buildability Dashboard")
        st.write("Total parcels processed:", len(parcels_3857))
        st.write("Average score:", round(parcels_3857["buildability_score"].mean(), 2))
        st.write("Min score:", round(parcels_3857["buildability_score"].min(), 2))
        st.write("Max score:", round(parcels_3857["buildability_score"].max(), 2))

        st.subheader("Top 50 Parcels (by buildability score)")
        st.dataframe(top50[["buildability_score"]].head(50))

        st.subheader("Map of Parcels (colored by score not yet, but spatially plotted)")
        st.map(parcels_4326[["geometry"]])
